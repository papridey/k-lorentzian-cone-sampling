...
def save_method_overlay(chains_stats_dict, outdir, col, bins=60):
    """
    Cross-method pooled histogram overlay for a scalar observable `col`.

    chains_stats_dict: dict[str, list[pd.DataFrame]]
        e.g. {"geom_MALA":[df1,df2,...], "naive_Euclid_drift_in_S":[...]}
    """
    import os
    import numpy as np
    import matplotlib.pyplot as plt

    def pooled_values(chains_stats_list, colname):
        return np.concatenate([df[colname].values for df in chains_stats_list], axis=0)
    ...
    plt.savefig(os.path.join(plots_dir, f"overlay_methods_{col}.png"), dpi=200)
    plt.close()    